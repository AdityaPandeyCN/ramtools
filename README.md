...
converts on N threads (default 1). With more than one thread the input is cut
into 64 MB blocks and every thread parses, encodes and compresses its own
blocks through ROOT's `RNTupleParallelWriter`; the clusters are appended in
input order, so the records keep their order and a coordinate-sorted input
still gives a file that region queries seek in. The records are the same
whatever the thread count, the cluster layout is not (one cluster per block
instead of one per 128 MB compressed). Memory use is about 2 * N * 64 MB on
top of ROOT's own. With `-split`, the threads compress pages only.
`bamtoramntuple input.bam output.root` converts BAM and takes the same options
apart from `-split`.
